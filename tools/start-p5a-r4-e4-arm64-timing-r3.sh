...
NAME=p5a-r4-e4-arm64-timing-r3
RUN_ID=20260721T-p5a-r4-e4-arm64-timing-r3
...
FAILURE_CLOSURE_RUNNER="$ROOT/capsched/capsched-models/validation/run-sched-exec-lease-p5a-r4-e4-arm64-timing-failure-closure.sh"
...
FAILURE_CLOSURE_ROOT="$ROOT/build/source-check/sched-exec-lease-p5a-r4-e4-arm64-timing-failure-closure"
FAILURE_CLOSURE_R1="$FAILURE_CLOSURE_ROOT/20260721T-p5a-r4-e4-arm64-timing-r2-failure-closure-r1"
FAILURE_CLOSURE_R2="$FAILURE_CLOSURE_ROOT/20260721T-p5a-r4-e4-arm64-timing-r2-failure-closure-r2"
CONFIG_SMOKE="$ROOT/build/source-check/sched-exec-lease-p5a-r4-e4-arm64-local-quantum-measurement/20260721T-p5a-r4-e4-arm64-timing-config-smoke-r7"
...
CAPSCHED_COMMIT=195523ae64777687b7e6627dfe1886a046d73c4e
WATCH=0
PREFLIGHT_ONLY=0
...
for script in "$PROBE" "$WRAPPER" "$RUNNER" "$QMP_CONTROL" "$QMP_TEST" \
	...
	"$SOURCE_CLOSURE_TEST" "$FAILURE_CLOSURE_RUNNER"; do
...
[ "$(file_sha "$RUNNER")" = 8b7ae0d18636f5027fd741527b0b5d1b8b5c5323fb62272c3a47cb5d17942fb2 ] || die 'timing runner hash changed'
...
[ "$(file_sha "$FAILURE_CLOSURE_RUNNER")" = 0610b76838bbd4eeb65625b490ac3bbb93331526b283b86d1969c6a71916881d ] || die 'r2 failure closure runner hash changed'

[ "$(file_sha "$SOURCE_CLOSURE_R1/result.json")" = 5e3ff71d2fea01b29e20b23a9bb8e1a8479d70cc847fa49aa3d33295c8040f3f ] || die 'source closure r1 result hash changed'
[ "$(file_sha "$SOURCE_CLOSURE_R2/result.json")" = bac2aca6649c40fdf21665a0f801be1f0751ef03c437d1b506f78ba77f04f720 ] || die 'source closure r2 result hash changed'
[ "$(file_sha "$SOURCE_CLOSURE_R1/result.normalized.json")" = 767d2f9ab1bfb6e0c918c2ba0b51147ba79f236085e6985097b14e5a8da43d21 ] || die 'source closure r1 normalized decision changed'
[ "$(file_sha "$SOURCE_CLOSURE_R2/result.normalized.json")" = 767d2f9ab1bfb6e0c918c2ba0b51147ba79f236085e6985097b14e5a8da43d21 ] || die 'source closure r2 normalized decision changed'
cmp "$SOURCE_CLOSURE_R1/result.normalized.json" "$SOURCE_CLOSURE_R2/result.normalized.json" >/dev/null || die 'source closure normalized decisions differ'
...
[ "$(file_sha "$FAILURE_CLOSURE_R1/result.json")" = 749777dbd6e9310538f76146650eca52d6c9d6c721645cb21c99cda196a0b705 ] || die 'failure closure r1 result hash changed'
[ "$(file_sha "$FAILURE_CLOSURE_R2/result.json")" = 83ec59df2275ab6d6b8c6a9fe2aed9ecb12156ae318b991a9fa1829d47b37e66 ] || die 'failure closure r2 result hash changed'
[ "$(file_sha "$FAILURE_CLOSURE_R1/result.normalized.json")" = 9c079b47fae1b7ae45baa6cd7517a9b02af2d6b3f4a9780f180366932e3178ef ] || die 'failure closure r1 normalized decision changed'
[ "$(file_sha "$FAILURE_CLOSURE_R2/result.normalized.json")" = 9c079b47fae1b7ae45baa6cd7517a9b02af2d6b3f4a9780f180366932e3178ef ] || die 'failure closure r2 normalized decision changed'
cmp "$FAILURE_CLOSURE_R1/result.normalized.json" "$FAILURE_CLOSURE_R2/result.normalized.json" >/dev/null || die 'failure closure normalized decisions differ'
[ -z "$(find "$FAILURE_CLOSURE_R1/inputs" "$FAILURE_CLOSURE_R2/inputs" -type f -perm -222 -print -quit)" ] || die 'failure closure inputs became writable'
...
[ "$(file_sha "$CONFIG_SMOKE/raw/measurement-runner.sh")" = 8b7ae0d18636f5027fd741527b0b5d1b8b5c5323fb62272c3a47cb5d17942fb2 ] || die 'config smoke used another runner'
...
[ "$(file_sha "$CLEANUP_NEGATIVE/raw/measurement-runner.sh")" = 8b7ae0d18636f5027fd741527b0b5d1b8b5c5323fb62272c3a47cb5d17942fb2 ] || die 'cleanup control used another runner'
[ "$(file_sha "$CLEANUP_NEGATIVE/raw/qmp-vcpu-control.py")" = e59bc8ad5adb50ddf66652b28a424afd1efbd28a9501e786771d5fb1f8da147e ] || die 'cleanup control used another QMP helper'
...
if [ "$PREFLIGHT_ONLY" = 1 ]; then
	printf 'preflight passed: pushed commits, exact source/failure closures, paused-QMP control, parser, smoke, cleanup, CPU, storage, and clean paths are r3 launch-ready\n'
	exit 0
fi
