...
validate_complete_result()
{
	...
	jq -e '
	  ...
	  .runner.sha256 == "8b7ae0d18636f5027fd741527b0b5d1b8b5c5323fb62272c3a47cb5d17942fb2" and
...
if [ -s "$RESULT" ]; then
	...
	printf 'failed\n'
	printf 'result=%s\n' "$RESULT"
	printf 'activity=result exists but exact paused-QMP arm64 evidence contract failed\n'
	exit 0
fi
...
activity=$(printf '%s\n' "$processes" |
	grep -E '[r]un-p5a-r4-e4-arm64-timing-r3-in-machine|[r]un-sched-exec-lease-p5a-r4-e4-arm64-local-quantum-measurement|[q]emu-system-aarch64|[m]ake -C /var/tmp/linux-cap-worktrees/p5a-r4-e4-arm64-measurement' |
	sed -n '1p')
if [ -n "$activity" ]; then
	printf 'running\n'
	printf 'machine_state=%s\n' "$machine_state"
